...
import { SQLLiteCake, SQLLiteCakeMapper } from "../../mappers/Cake.mapper";
...
export class CakeRepository implements IRepository<IdentifiableCake>, Initializable {
    ...
    async get(id: id): Promise<IdentifiableCake> {
         try {
            const conn = await ConnectionManager.getConnection();
            const result = await conn.get<SQLLiteCake>(SELECT_BY_ID, id);
            if (!result) {
                throw new ItemNotFoundException("Cake of id "+ id +" not found");
            }
            return new SQLLiteCakeMapper().map(result); 
        } catch (error) {
    ...
    async getAll(): Promise<IdentifiableCake[]> {
        try {
            const conn = await ConnectionManager.getConnection();
            const result = await conn.all<SQLLiteCake[]>(SELECT_ALL);
            const mapper = new SQLLiteCakeMapper();
